...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!matchedOrder) {
      matchedOrder = orders.find(o => o.status === 'pending');
    }
    ...
    if (matchedOrder) {
      ...
      // Gửi Email Xác Nhận Đơn Hàng & Bàn Giao Ebook
      try {
        const customers = await getCollection('customers');
        const cust = customers.find(c => c.id == matchedOrder.customer_id || (c.phone && c.phone === matchedOrder.customer_phone));
        const targetEmail = matchedOrder.customer_email || (cust ? cust.email : '') || 'haotrankha53@gmail.com';
        ...
        <div style="font-family: 'Plus Jakarta Sans', Arial, sans-serif; line-height: 1.6; color: #1E293B; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #E2E8F0; border-radius: 12px; background: #FFFFFF;">
          ...
          <p>Chào bạn <strong>${matchedOrder.customer_name || ''}</strong>,</p>
          ...
          <div style="background: #F8FAFC; border: 1px solid #E2E8F0; border-radius: 8px; padding: 18px; margin: 20px 0;">
            <p style="margin: 0 0 8px 0;">📦 <strong>Mã đơn hàng:</strong> <span style="color: #06B6D4; font-family: monospace; font-weight: bold;">#${matchedOrder.order_code || matchedOrder.id}</span></p>
            <p style="margin: 0 0 8px 0;">📚 <strong>Sản phẩm:</strong> ${matchedOrder.product_name || 'Ebook 4 Bước Luyện Tai Chữa Dứt Điểm Nghe Trôi Chữ'}</p>
            <p style="margin: 0;">💵 <strong>Số tiền:</strong> ${(matchedOrder.amount || 2000).toLocaleString('vi-VN')} VNĐ</p>
          </div>
        ...
        await fetch('https://api.resend.com/emails', {
          ...
          body: JSON.stringify({
            ...
            to: [targetEmail],
            subject: `[Xác Nhận Đơn Hàng #${matchedOrder.order_code || matchedOrder.id}] Bàn giao Ebook Luyện Tai 2K thành công! 📚⚡`,
            html: orderEmailHtml
          })
        });
        console.log(`✉️ [SePay Webhook Email] Đã gửi mail bàn giao đơn hàng #${matchedOrder.order_code} tới ${targetEmail}`);
      } catch(e) {
        console.error('Lỗi gửi mail đơn hàng SePay:', e);
      }
